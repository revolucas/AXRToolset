UIArtefactPropsWnd = nil
function OnApplicationBegin()
	UIMainMenuWnd:AddPluginButton("Artefact Properties","UIArtefactPropsShow",GetAndShow)
end

function Get()
	if not (UIArtefactPropsWnd) then 
		UIArtefactPropsWnd = cUIArtefactProps("1")
		UIArtefactPropsWnd.parent = UIMainMenuWnd
	end 
	return UIArtefactPropsWnd
end
...
cUIArtefactProps = Class{__includes={cUIBase}}
function cUIArtefactProps:init(id)
...
function cUIArtefactProps:Reinit()
	...
	self:Gui("Add|Button|gOnScriptControlAction x485 y680 w150 h20 vUIArtefactPropsExecute|Create artefact_props.txt")
...
function cUIArtefactProps:Arte(i_path)
	...
	Msg("artefact_props:= Finished!")
	
	local f = io.open("artefact_props.txt","wb+")
	if (f) then 
		f:write(str)
		f:close()
	end
