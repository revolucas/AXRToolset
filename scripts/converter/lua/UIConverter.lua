...
function OnApplicationBegin()
	UIMain.AddPluginButton("Converter","UIConverterShow",GetAndShow)
end
---------------------------------------------------------------------------
UI = nil
function Get()
...
cUIConverter = Class("cUIConverter",inherited)
		...
		self:Gui("Tab|%s",tabs[i])
			...
			if (Checks[i_s]) then
				...
				for n=1,#Checks[i_s] do
					self:Gui("Add|CheckBox|x50 y%s w100 h22 %s vUIConverterCheck%s%s|%s",y,gSettings:GetValue("converter","check_"..Checks[i_s][n]..i_s,"") == "1" and "Checked" or "",Checks[i_s][n],i_s,Checks[i_s][n])
					y = y + 20
				end
			end
			...
			self:Gui("Add|Button|gOnScriptControlAction x485 y655 w90 h20 vUIConverterSaveSettings%s|Save Settings",i)	
			self:Gui("Add|Button|gOnScriptControlAction x485 y680 w90 h20 vUIConverterExecute%s|Execute",i)		
