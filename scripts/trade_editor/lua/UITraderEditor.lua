...
function OnApplicationBegin()
	UIMain.AddPluginButton("Trader Editor","UITraderEditorShow",GetAndShow)
end

UI = nil
function Get()
	if not (UI) then 
		UI = cUITraderEditor:new("1")
...
cUITraderEditor = Class("cUITraderEditor",inherited)
...
function cUITraderEditor:FillListView(typ)
	...
	if (typ == "1") then
		...
		if not (self.ltx[fname]) then 
			self.ltx[fname] = cIniFile:new(dir.."\\configs\\misc\\"..fname,true)
		end
		if (self.ltx[fname]) then 
...
function GetModify()
	if not (UI2) then 
		UI2 = cUITraderEditorModify:new("2")
	end
	return UI2
end
...
-----------------------------------------------------------------
cUITraderEditorModify = Class("cUITraderEditorModify",inherited)
