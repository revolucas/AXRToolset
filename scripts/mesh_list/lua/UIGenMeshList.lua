...
function OnApplicationBegin()
	UIMain.AddPluginButton("Generate Mesh List","UIGenMeshListShow",GetAndShow)
end

UI = nil
function Get()
	if not (UI) then 
		UI = cUIGenMeshList:new("1")
...
cUIGenMeshList = Class("cUIGenMeshList",inherited)
...
function OnGenerate()
	...
	local outfile = cIniFile:new(".\\mesh_list.txt",true)
	if not (outfile) then 
	...
	local texturefile = cIniFile:new(".\\texture_list.txt",true)
